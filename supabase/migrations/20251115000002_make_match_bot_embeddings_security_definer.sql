-- Make match_bot_embeddings function SECURITY DEFINER
-- This allows the function to bypass RLS when called with admin/service role client
-- The function already has built-in security checks via team_id and bot_id parameters
...
IS 'Returns the most relevant chunks for a query embedding scoped to a team/bot combination. Uses SECURITY DEFINER to bypass RLS when called with admin/service role client. Security is enforced via team_id and bot_id parameters.';
