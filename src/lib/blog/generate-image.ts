 ...
 * 
 * Generates retro 8-bit pixel art feature images for blog posts
 */
...
async function generateImageWithDALLE(prompt: string): Promise<string> {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `DALL-E API error: ${response.status} ${errorText}`
    );
  }
...
export async function generateBlogImage(
  ...
  const imagePath = `/blog/images/${options.slug}.jpg`;
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.log(`[Image Generation] Attempt ${attempt}/${maxRetries} for ${options.slug}`);
      ...
      // Download and save
      await downloadAndSaveImage(imageUrl, publicPath);

      console.log(`[Image Generation] ✅ Successfully generated image for ${options.slug}`);
  ...
  const placeholderPath = path.join(process.cwd(), "public/blog/images/placeholder.svg");
  try {
  ...
  return {
    imagePath: "/blog/images/placeholder.svg", // Placeholder image
    status: "failed",
