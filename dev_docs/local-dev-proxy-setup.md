...
Requirements
- Main app running locally on http://localhost:3000 (or the URL you set).
- Shell app running locally on http://localhost:3001.
- Both apps use the same Supabase project.

Configuration
- Set NEXT_PUBLIC_APP_URL=http://localhost:3001 in .env.local for the shell.
- Set NEXT_PUBLIC_MAIN_APP_URL=http://localhost:3000 in .env.local for the shell.
- Restart the shell dev server after changing .env.local.

Auth Notes
- Sign in through the shell app to set auth cookies for localhost:3001.
